    ...
    -->
		<el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" :before-close="cancelDialog">
			<el-form :model="dialogForm" status-icon :rules="dialogFormRules"  ref="dialogForm">
				...
				<el-form-item label="星级" label-width="200px" prop="star">
					<el-rate
						v-model="dialogForm.star"
						:colors="starColors">
					</el-rate>
				</el-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      // 验证关键词
      let validWords = (rule, value, callback) => {
        console.log(value);
        if (value === '') {
      ...
      return {
        ...
        dialogFormRules: {
          word : [
            {required: true,  validator: validWords, trigger: 'blur'},
          ],
          star : [
            {required: true,   validator: validRates, trigger: 'change'},
          ],
          status : [
    ...
    methods: {
      ...
      getList() {
        ...
        getEvaluateKeyword(params).then((res) => {
      ...
      dialogClick() {
        this.$refs.dialogForm.validate((valid) => {
          if (valid) {
            ...
            saveOrEditKeyword(this.dialogForm).then(res => {
              if(res.code == 200) {
                this.cancelDialog();
                this.getList();
                this.$notify.success({
