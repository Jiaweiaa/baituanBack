<template>
  <div class="labberConcessionsBox">
    ...
    <!--
			新增热销
    -->
    <el-dialog title="新增热销" :visible.sync="dialogFormVisible" :before-close="cancelDialog">
      <el-form
        :model="dialogForm"
...
export default {
            ...
            return (
              <img
                ...
                src={row.itemDto!=null?
                  "https://static.ibaituan.cn/" + row.itemDto.itemImages[0].picUrl:""
                }
              />
            );
          }
  ...
  methods: {
    ...
    getList() {
      ...
      getHotSaleList(params).then(res => {
    ...
    dialogClick() {
      this.$refs.dialogForm.validate(valid => {
        if (valid) {
          ...
          saveOrUpdateHotSale(this.dialogForm).then(res => {
            if (res.code == 200) {
              this.cancelDialog();
              this.$notify.success({
                title: res.message,
        ...
        .then(() => {
          ...
          deleteHotSale(params).then(res => {
            if (res.code == 200) {
              ...
              getHotSaleList(params).then(res => {
                // console.log(this.searchParams,'777');
                if (
