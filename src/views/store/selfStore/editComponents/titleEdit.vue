			...
			<el-form-item
				...
				<div v-if="index == '4'">
					<el-button
						...
						plain
						v-if="isShowBind"
						type="success">添加一个文本导航</el-button>
					<div class="imgBox" v-else>
						<i @click="clearBind" class="boxIcon el-icon-circle-close"></i>
						<el-input class="input" size="small" v-model="dialogForm['navigation'].name" placeholder="商品标题"></el-input>
						<el-select @change="goodsChange(goodsIndex)" class="select" size="small" v-model="dialogForm['navigation'].type" placeholder="请选择跳转">
							<el-option
						...
						<div class="goodsBindFor" v-show="(typeData == 2 || typeData == 4) && dialogForm['navigation'].data.id || dialogForm['navigation'].data.name">
							{{dialogForm['navigation'].data.propertySetName}} {{dialogForm['navigation'].data.categoryNames}}{{dialogForm['navigation'].data.name}}
						</div>
					</div>
				</div>
			</el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 选中取消
      dialogClick() {
        if(this.dialogForm['navigation'].data){
          if(this.typeData == 1) {
            this.dialogForm['navigation'].data = this.goodsSelected;
          } else if(this.typeData == 2) {
            this.dialogForm['navigation'].data = this.goodsSelected;
          } else if(this.typeData == 4) {
            this.dialogForm['navigation'].data= this.goodsSelected;
            this.dialogForm['navigation'].data.propertySetName = this.goodsSelected.title;
          }
      ...
      goodsChange(goodsIndex) {
        ...
        this.typeData = '';
        this.dialogForm['navigation'].data = {};
        if(this.dialogForm['navigation'].type == 1) {
          this.typeData = 1;
        ...
        if(this.dialogForm['navigation'].type == 2) {
          this.typeData = 2;
          this.dialogFormVisible = true;
        ...
        if(this.dialogForm['navigation'].type == 4) {
          this.typeData = 4;
          this.dialogFormVisible = true;
