  ...
  <div class="labberConcessionsBox">
    ...
    <el-table
      ...
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.itemNum ? scope.row.itemNum  : ''}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="有效期"
      ...
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <span>{{scope.row.channelNames}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="状态"
...
<script>
  ...
  import { bundleList, stopBundle, deleteBundle } from '@/api/promotion/bundling';
    ...
    created() {
      ...
      channelList().then(res => {
        if(res.code == 200) {
          ...
          this.searchModel[0].input[2].options = this.channelListData;
        }
      })
    },
    methods: {
      ...
      del(val) {
        ...
        .then(() => {
          ...
          this.options.loading = true;
          deleteBundle(params).then((res) => {
            if(res.code == 200) {
              bundleList(this.searchData).then((res) => {
                this.options.loading = false;
                this.$message({
                  type: "success",
                  message: "删除成功!"
