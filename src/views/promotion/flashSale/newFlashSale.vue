 ...
 * @LastEditTime: 2019-09-10 16:49:38
	...
	<div class="information">
		...
		<el-form
			...
			<el-form-item label="活动名称" label-width="180px" prop="offerName">
				<el-input :disabled="pageTitle == '查看限时抢购'" v-model.trim="dialogForm.offerName"></el-input>
			</el-form-item>
			<el-form-item label="活动文案" label-width="180px" prop="marketingMessage">
				<el-input :disabled="pageTitle == '查看限时抢购'" type="textarea" v-model.trim="dialogForm.marketingMessage"></el-input>
			</el-form-item>
			<el-form-item label="开始时间" label-width="180px" prop="startDate">
				<el-date-picker
					:disabled="pageTitle !== '新增限时抢购'"
			...
			<el-form-item label="发布渠道" label-width="180px" prop="channelIds">
				<el-checkbox-group :disabled="pageTitle == '查看限时抢购'" v-model="dialogForm.channelIds">
					<el-checkbox v-for="channel in channelListData" :label="channel.id" :key="channel.id">{{channel.name}}</el-checkbox>
				</el-checkbox-group>
			</el-form-item>
			<el-form-item
				v-show="dialogForm.status !== 2 ? false : $route.query.title.indexOf('编辑限时抢购') !== -1 ? true : false"
				label="操作"
				label-width="180px">
			...
			<el-form-item label="活动自动取消订单时间" label-width="180px" prop="autoCancelTimeout">
				<el-input  :disabled="pageTitle == '查看限时抢购'" style="width: 20%;" v-model="dialogForm.autoCancelTimeout">
					<template slot="append">分钟</template>
				</el-input>
			</el-form-item>
			<el-form-item label=" 选择商品" label-width="180px" prop="itemSku">
				<el-button style="margin-bottom: 10px;" type="primary" size="small"  :disabled="pageTitle == '查看限时抢购'" @click="addGoods">添加商品</el-button>
						...
						<el-table
							...
							<el-table-column>
								<template slot="header" slot-scope="scope">
									限时抢购价<i style="cursor: pointer;" @click="saleSetClick(index)" class="el-icon-s-tools"></i>
								</template>
								<template slot-scope="scope">
									<el-input
										:disabled="pageTitle !== '新增限时抢购'"
										v-model="scope.row.discountedPrice"
										placeholder="限时抢购价"></el-input>
								</template>
							</el-table-column>
							<el-table-column>
								<template slot="header" slot-scope="scope">
									限时抢购库存 <i @click="inventorySetClick(index)"  style="cursor: pointer;" class="el-icon-s-tools"></i>
								</template>
								<template slot-scope="scope">
									<el-input
										:disabled="scope.row.availableInventory == '' ? true :  scope.row.availableInventory == null ? true: $route.query.title.indexOf('编辑限时抢购') !== -1 ? true: false"
										v-model="scope.row.availableInventory"
										placeholder="限时抢购库存"></el-input>
								</template>
							</el-table-column>
						</el-table>
					</el-collapse-item>
		...
		<div style="float: right;margin-right: 10%;" slot="footer" class="dialog-footer">
			<el-button v-show="pageTitle == '查看限时抢购'" @click="cancelDialog">关 闭</el-button>
			<el-button v-show="pageTitle !== '查看限时抢购'" @click="cancelDialog">取 消</el-button>
			<el-button v-show="pageTitle !== '查看限时抢购'" type="primary" :loading="btnLoading" @click="dialogClick">确 定</el-button>
		</div>
		...
		<!--
			限时抢购价dialog
		-->
		<el-dialog title="限时抢购价" :visible.sync="saleVisible" :before-close="cancelSaleDialog">
		...
		<!--
			限时抢购库存dialog
		-->
		<el-dialog title="限时抢购价" :visible.sync="buyVisible" :before-close="cancelBuyDialog">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 价格设置
      saleSetClick(val) {
        if(this.pageTitle !== '新增限时抢购') return;
        this.clickIndex = val;
      ...
      cancelBuyDialog() {
        ...
        this.buyVisible = false;
			...
			// 设置限购数
      setBuyNum(val) {
        if(this.pageTitle == '查看限时抢购') return;
        this.clickData = val;
      ...
      dialogClick() {
        this.$refs.dialogForm.validate((valid) => {
            ...
            saveLimitDiscount(this.dialogForm).then(res => {
              if(res.code == 200) {
                if(this.pageTitle == '新建限时抢购') {
                  this.$message({
                    type: "success",
                    message: "新建限时抢购成功!"
                  });
      ...
      cancelDialog() {
        this.$router.push({
          path: '/promotion/flashSaleTemplate/flashSale'
        });
      }
    }
  }
