    ...
    -->
		<el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" :before-close="cancelDialog">
			<el-form :model="dialogForm" status-icon :rules="dialogFormRules"  ref="dialogForm">
				<el-form-item label="字典名称" label-width="200px" prop="name">
					<el-input v-model="dialogForm.name"></el-input>
				</el-form-item>
				<el-form-item label="字典code" label-width="200px" prop="code">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      dialogClick() {
        this.$refs.dialogForm.validate((valid) => {
          if (valid) {
            ...
            saveDic(this.dialogForm).then(res => {
              if(res.code == 200) {
                this.cancelDialog();
                this.getList();
                this.$notify.success({
      ...
      toDetail(val) {
        this.$router.push({
          path: '/gaoshan/dictionaryTemplate/childDictionary',
	        query: {
            id: val.id
	        }
        });
